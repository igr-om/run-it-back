...
- **This hasn't been run against a live Postgres instance or load-tested.** Every crate has been verified to compile, type-check, and pass its test suite (see below), and the logic has been reviewed carefully, but "compiles and passes unit tests" isn't the same as "has survived production traffic." Treat this as a serious, working v1   not a project that's been battle-tested in the field yet.
